...
MavlinkInterface::MavlinkInterface ()
    : received_first_actuator_ (false), gcs_socket_fd_ (0), sdk_socket_fd_ (0),
      simulator_socket_fd_ (0), simulator_tcp_client_fd_ (0),
      serial_enabled_ (false), m_status{}, m_buffer{},
      mavlink_addr_str_ ("INADDR_ANY"),
      mavlink_udp_port_ (kDefaultMavlinkUdpPort),
      mavlink_tcp_port_ (kDefaultMavlinkTcpPort),
      gcs_udp_port_ (kDefaultGCSUdpPort), sdk_udp_port_ (kDefaultSDKUdpPort),
      gcs_addr_ ("INADDR_ANY"), sdk_addr_ ("INADDR_ANY"), io_service (),
      serial_dev (io_service), device_ (kDefaultDevice),
      baudrate_ (kDefaultBaudRate), tx_q{}, rx_buf{}, baro_updated_ (false),
      diff_press_updated_ (false), mag_updated_ (false)
{
}
...
void MavlinkInterface::Load ()
{
    ...
    if (mavlink_addr_str_ != "INADDR_ANY") {
        ...
        if (mavlink_addr_ == INADDR_NONE) {
            std::cerr << "Invalid mavlink_addr: " << mavlink_addr_str_
                      << ", aborting\n";
            abort ();
        }
    }
    ...
    if (hil_mode_) {
        ...
        if ((gcs_socket_fd_ = socket (AF_INET, SOCK_DGRAM, 0)) < 0) {
            std::cerr << "Creating GCS UDP socket failed: " << strerror (errno)
                      << ", aborting\n";
            abort ();
        }

        if (bind (gcs_socket_fd_, (struct sockaddr *)&local_gcs_addr_,
                  local_gcs_addr_len_) < 0) {
            std::cerr << "GCS UDP bind failed: " << strerror (errno)
                      << ", aborting\n";
            abort ();
        }

        if ((sdk_socket_fd_ = socket (AF_INET, SOCK_DGRAM, 0)) < 0) {
            std::cerr << "Creating SDK UDP socket failed: " << strerror (errno)
                      << ", aborting\n";
            abort ();
        }

        if (bind (sdk_socket_fd_, (struct sockaddr *)&local_sdk_addr_,
                  local_sdk_addr_len_) < 0) {
            std::cerr << "SDK UDP bind failed: " << strerror (errno)
                      << ", aborting\n";
            abort ();
        }
    }
    ...
    if (serial_enabled_) {
        ...
        memset ((char *)&remote_simulator_addr_, 0,
                sizeof (remote_simulator_addr_));
        remote_simulator_addr_.sin_family = AF_INET;
        remote_simulator_addr_len_ = sizeof (remote_simulator_addr_);

        memset ((char *)&local_simulator_addr_, 0,
                sizeof (local_simulator_addr_));
        ...
        if (use_tcp_) {
            ...
            if ((simulator_socket_fd_ = socket (AF_INET, SOCK_STREAM, 0)) < 0) {
                std::cerr << "Creating TCP socket failed: " << strerror (errno)
                          << ", aborting\n";
                abort ();
            }

            int yes = 1;
            int result = setsockopt (simulator_socket_fd_, IPPROTO_TCP,
            ...
            result = setsockopt (simulator_socket_fd_, SOL_SOCKET, SO_LINGER,
                                 &nolinger, sizeof (nolinger));
            if (result != 0) {
                std::cerr << "setsockopt failed: " << strerror (errno)
                          << ", aborting\n";
                abort ();
            ...
            // The socket reuse is necessary for reconnecting to the same
            // address if the socket does not close but gets stuck in TIME_WAIT.
            // This can happen if the server is suddenly closed
            int socket_reuse = 1;
            result = setsockopt (simulator_socket_fd_, SOL_SOCKET, SO_REUSEADDR,
            ...
            result = setsockopt (simulator_socket_fd_, SOL_SOCKET, SO_REUSEPORT,
                                 &socket_reuse, sizeof (socket_reuse));
            if (result != 0) {
                std::cerr << "setsockopt failed: " << strerror (errno)
                          << ", aborting\n";
            ...
            if (result == -1) {
                std::cerr << "setting socket to non-blocking failed: "
                          << strerror (errno) << ", aborting\n";
                abort ();
            }

            if (bind (simulator_socket_fd_,
                      (struct sockaddr *)&local_simulator_addr_,
                      local_simulator_addr_len_) < 0) {
            ...
            fds_[LISTEN_FD].events =
                POLLIN; // only listens for new connections on tcp
        }
        else {
            ...
            if ((simulator_socket_fd_ = socket (AF_INET, SOCK_DGRAM, 0)) < 0) {
                std::cerr << "Creating UDP socket failed: " << strerror (errno)
                          << ", aborting\n";
                abort ();
            }
...
void MavlinkInterface::SendSensorMessages (int time_usec)
{
    ...
    if (imu_updated_) {
        ...
        sensor_msg.fields_updated =
            (uint16_t)SensorSource::ACCEL | (uint16_t)SensorSource::GYRO;
    ...
    if (mag_updated_) {
        ...
        sensor_msg.fields_updated =
            sensor_msg.fields_updated | (uint16_t)SensorSource::MAG;
    ...
    if (baro_updated_) {
        ...
        sensor_msg.fields_updated =
            sensor_msg.fields_updated | (uint16_t)SensorSource::BARO;
    ...
    if (diff_press_updated_) {
        ...
        sensor_msg.fields_updated =
            sensor_msg.fields_updated | (uint16_t)SensorSource::DIFF_PRESS;
    ...
    if (!hil_mode_ || (hil_mode_ && !hil_state_level_)) {
        ...
        mavlink_msg_hil_sensor_encode_chan (1, 200, MAVLINK_COMM_0, &msg,
                                            &sensor_msg);
...
void MavlinkInterface::SendGpsMessages (const SensorData::Gps &data)
{
    ...
    if (!hil_mode_ || (hil_mode_ && !hil_state_level_)) {
        mavlink_message_t msg;
        mavlink_msg_hil_gps_encode_chan (1, 200, MAVLINK_COMM_0, &msg,
                                         &hil_gps_msg);
        send_mavlink_message (&msg);
    }
}
...
void MavlinkInterface::pollForMAVLinkMessages ()
{
    ...
    do {
        int timeout_ms =
            (received_first_actuator_ && enable_lockstep_) ? 1000 : 0;
        ...
        for (int i = 0; i < N_FDS; i++) {
            ...
            if (i == LISTEN_FD) { // if event is raised on the listening socket
                ...
                int ret = recvfrom (fds_[i].fd, _buf, sizeof (_buf), 0,
                                    (struct sockaddr *)&remote_simulator_addr_,
    ...
    } while (!close_conn_ && received_first_actuator_ && !received_actuator &&
             enable_lockstep_ && !gotSigInt_);
}
...
void MavlinkInterface::pollFromGcsAndSdk ()
{
    ...
    if (fds[0].revents & POLLIN) {
        int len = recvfrom (gcs_socket_fd_, _buf, sizeof (_buf), 0,
                            (struct sockaddr *)&remote_gcs_addr_,
                            &remote_gcs_addr_len_);

        if (len > 0) {
            mavlink_message_t msg;
            mavlink_status_t status;
            for (unsigned i = 0; i < len; ++i) {
                if (mavlink_parse_char (MAVLINK_COMM_1, _buf[i], &msg,
                                        &status)) {
    ...
    if (fds[1].revents & POLLIN) {
        int len = recvfrom (sdk_socket_fd_, _buf, sizeof (_buf), 0,
                            (struct sockaddr *)&remote_sdk_addr_,
                            &remote_sdk_addr_len_);

        if (len > 0) {
            mavlink_message_t msg;
            mavlink_status_t status;
            for (unsigned i = 0; i < len; ++i) {
                if (mavlink_parse_char (MAVLINK_COMM_2, _buf[i], &msg,
                                        &status)) {
...
void MavlinkInterface::acceptConnections ()
{
    ...
    int ret =
        accept (fds_[LISTEN_FD].fd, (struct sockaddr *)&remote_simulator_addr_,
                &remote_simulator_addr_len_);
...
void MavlinkInterface::forward_mavlink_message (
    const mavlink_message_t *message)
{
    ...
    if (gcs_socket_fd_ > 0) {
        len =
            sendto (gcs_socket_fd_, buffer, packetlen, 0,
                    (struct sockaddr *)&remote_gcs_addr_, remote_gcs_addr_len_);

        if (len <= 0) {
            std::cerr << "Failed sending mavlink message to GCS: "
                      << strerror (errno) << "\n";
        }
    }
    ...
    if (sdk_socket_fd_ > 0) {
        len =
            sendto (sdk_socket_fd_, buffer, packetlen, 0,
                    (struct sockaddr *)&remote_sdk_addr_, remote_sdk_addr_len_);
        if (len <= 0) {
            std::cerr << "Failed sending mavlink message to SDK: "
                      << strerror (errno) << "\n";
        }
    }
}
    ...
    if (serial_enabled_) {
        ...
        if (fds_[CONNECTION_FD].fd > 0) {
            ...
            if (!(fds_[CONNECTION_FD].revents & POLLOUT)) {
                std::cerr << "invalid events at fd:"
                          << fds_[CONNECTION_FD].revents << "\n";
                return;
            }
            ...
            if (use_tcp_) {
                ...
                len = sendto (fds_[CONNECTION_FD].fd, buffer, packetlen, 0,
                              (struct sockaddr *)&remote_simulator_addr_,
                              remote_simulator_addr_len_);
            }
            if (len < 0) {
                std::cerr << "Failed sending mavlink message: "
                          << strerror (errno) << "\n";
...
void MavlinkInterface::do_read (void)
{
    serial_dev.async_read_some (
        ...
        boost::bind (&MavlinkInterface::parse_buffer, this,
                     boost::asio::placeholders::error,
                     boost::asio::placeholders::bytes_transferred));
}

// Based on MAVConnInterface::parse_buffer in MAVROS
void MavlinkInterface::parse_buffer (const boost::system::error_code &err,
                                     std::size_t bytes_t)
{
    mavlink_status_t status;
